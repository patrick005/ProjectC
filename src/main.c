...
// #define PIR_PIN         PC5         // PIR 센서 출력 연결 핀 (이제 감지 시 HIGH)
// // #define CDS_CHANNEL     0           // CdS 센서가 연결된 ADC 채널 (PF0)
// #define CDS_CHANNEL     4           // CdS 센서가 연결된 ADC 채널 (PF0 -> pc4)  ***변경해야함***
...
//         PORTF &= ~(1 << PC1); // 빨강 채널 ON (LOW)
//         PORTF &= ~(1 << PC2); // 초록 채널 ON (LOW)
//         PORTF |=  (1 << PC3); // 파랑 채널 OFF (HIGH)
//     } else {
//         // 조건 미충족 시 LED OFF (모든 채널 HIGH)
//         PORTF |= (1 << PC1) | (1 << PC2) | (1 << PC3);
//     }
